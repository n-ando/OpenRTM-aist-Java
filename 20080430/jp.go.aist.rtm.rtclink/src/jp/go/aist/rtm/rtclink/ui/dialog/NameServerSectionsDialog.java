package jp.go.aist.rtm.rtclink.ui.dialog;
...
import jp.go.aist.rtm.rtclink.manager.NameServerManager;
...
public class NameServerSectionsDialog extends Dialog {
	...
	private void loadDefaultComboValue(Combo combo) {
		String defaultString = RtcLinkPlugin.getDefault().getPreferenceStore()
		...
		combo.select(RtcLinkPlugin.getDefault().getPreferenceStore().getInt(
	...
	private void addDefaultComboValue(Combo combo) {
		...
		if (valueList.contains(value) == false) {
			String defaultString = RtcLinkPlugin.getDefault()
