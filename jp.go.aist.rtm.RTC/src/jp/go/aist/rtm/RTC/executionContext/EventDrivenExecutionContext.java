...
/**
 * <p>Stimulus Response Processing(FSM用)ExecutionContextクラスです。</p>
 */

public class EventDrivenExecutionContext 
...
implements Runnable, ObjectCreator<ExecutionContextBase>, ObjectDestructor, ExecutionContextBase {
    ...
    public EventDrivenExecutionContext() {
        super();
        rtcout = new Logbuf("EventDrivenExecutionContext");
        m_svc = false;
        m_nowait = false;
    ...
    public ExecutionContextService getRef() {
        ...
        rtcout.println(Logbuf.TRACE, "EventDrivenExecutionContext.getRef()");
    ...
    public int open() {
        ...
        rtcout.println(Logbuf.TRACE, "EventDrivenExecutionContext.open()");
        if(m_thread==null) {
            m_thread = new Thread(this, "EventDrivenExecutionContext");
            m_thread.start();
        }
        return 0;
    }
    ...
    public int svc() {
        int count = 0 ;
        rtcout.println(Logbuf.TRACE, "EventDrivenExecutionContext.svc()");
    ...
    public void run() {
        ...
        rtcout.println(Logbuf.TRACE, "EventDrivenExecutionContext.run()");
    ...
    public int close(long flags) {
        ...
        rtcout.println(Logbuf.TRACE, "EventDrivenExecutionContext.close()");
    ...
    public boolean isRunning() {
        ...
        rtcout.println(Logbuf.TRACE, "EventDrivenExecutionContext.isRunning()");
...
//        return ReturnCode_t.RTC_OK;
    ...
    public ReturnCode_t onActivated(RTObjectStateMachine comp, long count)
    {
        rtcout.println(Logbuf.TRACE, 
                "EventDrivenExecutionContext.onActivated(count = " + count +")");
...
//                         "curr: " + comp.getStatus() +", next: " + comp.getStatus());
    ...
    {
        rtcout.println(Logbuf.TRACE, 
                "EventDrivenExecutionContext.onDeactivated(count = " + count +")");
//                rtcout.println(Logbuf.PARANOID, 
//                         "curr: " + comp.getStatus() +", next: " + comp.getStatus());
        // Now comp's next state must be ACTIVE state
        // If worker thread is stopped, restart worker thread.
    ...
    {
        rtcout.println(Logbuf.TRACE, 
                "EventDrivenExecutionContext.onReset(count = " + count +")");
//                rtcout.println(Logbuf.PARANOID, 
//                         "curr: " + comp.getStatus() +", next: " + comp.getStatus());
        // Now comp's next state must be ACTIVE state
        // If worker thread is stopped, restart worker thread.
    ...
    {
        rtcout.println(Logbuf.TRACE, 
        "EventDrivenExecutionContext.onWaitingActivated(count = " + count +")");
//        rtcout.println(Logbuf.PARANOID, 
//                 "curr: " + comp.getStatus() +", next: " + comp.getStatus());
      // Now comp's next state must be ACTIVE state
      // If worker thread is stopped, restart worker thread.
    ...
    {
        rtcout.println(Logbuf.TRACE, 
                "EventDrivenExecutionContext.onWaitingDeactivated(count = " + count +")");
        if(isAllNextState(LifeCycleState.INACTIVE_STATE))
        {
            synchronized (m_workerthread.mutex_) {
                if(m_workerthread.running_ == true)
    ...
    {
        rtcout.println(Logbuf.TRACE, 
                "EventDrivenExecutionContext.onWaitingReset(count = " + count +")");
        if(isAllNextState(LifeCycleState.INACTIVE_STATE))
        {
            synchronized (m_workerthread.mutex_) {
                if(m_workerthread.running_ == true)
    ...
    public ReturnCode_t activateComponent(LightweightRTObject comp) {
        ...
        rtcout.println(Logbuf.TRACE, "EventDrivenExecutionContext.activateComponent()");
    ...
    {
        rtcout.println(Logbuf.TRACE,"EventDrivenExecutionContext.deactivateComponent()");
    ...
    public ReturnCode_t resetComponent(LightweightRTObject comp){
        ...
        rtcout.println(Logbuf.TRACE, "EventDrivenExecutionContext.resetComponent()");
    ...
    public LifeCycleState getComponentState(LightweightRTObject comp) {
        ...
        rtcout.println(Logbuf.TRACE, "EventDrivenExecutionContext.getComponentState() = " + getStateString(state));
    ...
    public ExecutionKind getKind()
    {
        ...
        rtcout.println(Logbuf.TRACE, "EventDrivenExecutionContext.getKind() = " + getKindString(kind));
        kind = onGetKind(kind);
    ...
    public ReturnCode_t add_component(LightweightRTObject comp) {
        ...
        rtcout.println(Logbuf.TRACE, 
                            "EventDrivenExecutionContext.add_component()");
        return addComponent(comp);
    }
    public ReturnCode_t addComponent(LightweightRTObject comp) {

        rtcout.println(Logbuf.TRACE, 
                            "EventDrivenExecutionContext.addcomponent()");
    ...
    public ReturnCode_t bindComponent(RTObject_impl rtc) {
        ...
        rtcout.println(Logbuf.TRACE, 
                    "EventDrivenExecutionContext.bindComponent()");
    ...
    public ReturnCode_t remove_component(LightweightRTObject comp) {
        ...
        rtcout.println(Logbuf.TRACE, 
                        "EventDrivenExecutionContext.remove_component()");
        return removeComponent(comp);
    }
    public ReturnCode_t removeComponent(LightweightRTObject comp) {

        rtcout.println(Logbuf.TRACE, 
                        "EventDrivenExecutionContext.removeComponent()");
    ...
    public RTC.ExecutionContextProfile get_profile() {
        ...
        rtcout.println(Logbuf.TRACE, "EventDrivenExecutionContext.get_profile()");
...
*/
     ...
     */
    public static void EventDrivenExecutionContextInit(Manager manager) {
        //manager.registerECFactory("jp.go.aist.rtm.RTC.executionContext.EventDrivenExecutionContext");
        ExecutionContextFactory<ExecutionContextBase,String> factory 
                                        = ExecutionContextFactory.instance();
        factory.addFactory("jp.go.aist.rtm.RTC.executionContext.EventDrivenExecutionContext",
                    new EventDrivenExecutionContext(),
                    new EventDrivenExecutionContext());
    }

    /**
     * {@.ja EventDrivenExecutionContext を生成する}
     * {@.en Creats EventDrivenExecutionContext}
     * 
     * @return 
     *   {@.ja 生成されたinstance}
    ...
    public ExecutionContextBase creator_() {
        return new EventDrivenExecutionContext();
    }
    /**
     * {@.ja Object を破棄する}
     * {@.en Destructs Object}
    ...
    public RTC.ExecutionContextProfile getProfile(){
        rtcout.println(Logbuf.TRACE, "EventDrivenExecutionContext.getProfile()");
    ...
    {
        rtcout.println(Logbuf.TRACE, "EventDrivenExecutionContext.setTimeout(" + key +")");
