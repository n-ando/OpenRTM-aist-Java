...
public class ConfigFormPage extends FormPage {
	...
	protected void createFormContent(IManagedForm managedForm) {
		...
		configurationSetTableViewer = createConfigurationSetSelection(managedForm, form,
	...
	private String validateRtcParam() {
		...
		if (result == null && 
				serviceProviderIdlText.getText().length() > 0 &&
				((ArrayList)serviceProviderParamTableViewer.getInput()).size() == 0 ) {
			result = "Service Provider Port を入力してください。";			
		}
		if (result == null && 
				serviceConsumerIdlText.getText().length() > 0 &&
				((ArrayList)serviceConsumerParamTableViewer.getInput()).size() == 0 ) {
			result = "Service Consumer Port を入力してください。";			
		}
	...
	private boolean checkServicePortDefinition() {
		return false;
	}
	...
	private TableViewer createConfigurationSetSelection(IManagedForm managedForm,
		...
		Composite configurationSetComposite = managedForm.getToolkit()
				.createComposite(composite, SWT.NULL);
		configurationSetComposite.setLayoutData(gd);
		gl = new GridLayout(3, false);
		configurationSetComposite.setData(FormToolkit.KEY_DRAW_BORDER,
				FormToolkit.TEXT_BORDER);
		managedForm.getToolkit().paintBordersFor(configurationSetComposite);
		configurationSetComposite.setLayout(gl);

		Label configurationSetsLabel = managedForm.getToolkit().createLabel(
				configurationSetComposite, "Cfg Sets:");
		gd = new GridData(GridData.VERTICAL_ALIGN_BEGINNING);
		gd.widthHint = 50;
		configurationSetsLabel.setLayoutData(gd);

		final TableViewer configSetTableViewer = new TableViewer(managedForm
				.getToolkit().createTable(configurationSetComposite,
		...
		configSetTableViewer.setColumnProperties(new String[] {
				CONFIGSET_PROPERTY_NAME, CONFIGSET_PROPERTY_TYPE, CONFIGSET_PROPERTY_DEFAULT });
		...
		Composite buttonComposite = managedForm.getToolkit().createComposite(
				configurationSetComposite, SWT.NONE);
		gl = new GridLayout();
		gl.marginRight = 0;
		buttonComposite.setLayout(gl);
		gd = new GridData(GridData.FILL_BOTH);
	...
	private Object[] createServiceSelection(IManagedForm managedForm,
		...
		Label caution = managedForm.getToolkit().createLabel(
				serviceParamComposite, "※コード生成後，指定したIDLファイルをOutput directory内にコピーしてください");
		gd = new GridData(GridData.FILL_HORIZONTAL);
		gd.horizontalSpan = 3;
		caution.setLayoutData(gd);
...
//		cppWinCheck = createCheckboxAndLabel(toolkit, composite, "C++(Windows)");
	...
	private void update() {
		...
		if (cppCheck.getSelection()) {
			...
			rtcParam.setRtmVersion(RtcParam.RTM_VERSION_042);
		}
//		if (cppWinCheck.getSelection()) {
//			rtcParam.getLangList().add(RtcParam.LANG_CPPWIN);
//		}
		if (pythonCheck.getSelection()) {
			rtcParam.getLangList().add(RtcParam.LANG_PYTHON);
			rtcParam.setRtmVersion(RtcParam.DEFAULT_RTM_VERSION);
		}
...
//		cppWinCheck.setSelection(rtcParam.getLangList()
//				.contains(RtcParam.LANG_CPPWIN));
